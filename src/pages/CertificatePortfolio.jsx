...
const CertificatePortfolio = () => {
      ...
      <div className="min-h-screen bg-black flex items-center justify-center p-4">
        <div className="flex flex-col items-center gap-6">
          <div className="relative w-16 h-16">
            <div className="absolute inset-0 border-4 border-[#222222]/50 rounded-full"></div>
            <div className="absolute inset-0 border-4 border-yellow-500 border-t-transparent rounded-full animate-spin"></div>
          </div>
          <p className="text-gray-400 font-bold text-xs tracking-[0.3em] uppercase">Verifying Certificate Integrity...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center p-4">
        <motion.div
          ...
          className="bg-[#111111]/80 backdrop-blur-md p-12 rounded-[3rem] shadow-[0_8px_30px_rgb(0,0,0,0.3)] border border-[#222222]/50 max-w-sm w-full text-center"
        >
          ...
          <button onClick={() => window.location.reload()} className="w-full py-4 bg-gradient-to-r from-yellow-400 to-yellow-500 text-white font-bold rounded-2xl hover:from-yellow-300 hover:to-yellow-400 transition-all shadow-lg shadow-yellow-500/20">
            Retry Verification
          </button>
        </motion.div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-black text-gray-100 font-sans overflow-x-hidden selection:bg-yellow-500/30 selection:text-violet-200 relative print:bg-white print:text-black">
      {notify && (
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0 no-print">
        <div className="absolute top-[-20%] right-[-20%] w-[60%] h-[60%] bg-yellow-500/15 blur-[150px] rounded-full"></div>
        <div className="absolute bottom-[-20%] left-[-20%] w-[50%] h-[50%] bg-yellow-500/15 blur-[150px] rounded-full"></div>
      </div>
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 py-8 md:py-16 flex flex-col items-center">
        ...
        >
          <div className="flex items-center gap-3 bg-[#111111]/90 backdrop-blur-md px-6 py-3 rounded-full border border-yellow-500/20 shadow-[0_8px_30px_rgb(0,0,0,0.5)] mb-4">
            <div className="w-8 h-8 bg-gradient-to-tr from-yellow-400 to-yellow-500 rounded-lg flex items-center justify-center shadow-lg shadow-yellow-500/30">
          ...
          <div className="bg-yellow-500/15 text-yellow-300 px-5 py-2 rounded-full text-xs font-black uppercase tracking-[0.2em] border border-yellow-500/30 shadow-[0_0_20px_rgba(139,92,246,0.2)] flex items-center gap-2 animate-pulse">
        ...
        >
          <div ref={certificateRef} className="w-full bg-gray-900/50 p-2 md:p-4 rounded-2xl border border-gray-800/80 shadow-2xl backdrop-blur-sm print:bg-white print:p-0 print:border-none flex items-center justify-center">
            <CertificateSVG 
        ...
        >
          {/* Main info panel */}
          <div className="md:col-span-7 bg-[#111111]/80 backdrop-blur-md rounded-3xl border border-[#222222]/50 p-8 flex flex-col justify-between shadow-[0_8px_30px_rgb(0,0,0,0.3)]">
            <div className="flex items-center gap-4 mb-6">
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-yellow-400 to-yellow-500 flex items-center justify-center shadow-lg shadow-yellow-500/20 flex-shrink-0">
                <Award className="w-6 h-6 text-white" />
              </div>
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-gray-500">Certificate Awarded To</p>
                <h2 className="text-2xl font-black text-white tracking-tight leading-none">{cert.candidateName}</h2>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="bg-black/60 p-4 rounded-2xl border border-[#222222]/40">
                <div className="flex items-center gap-2 mb-1">
                  <Building2 className="w-3.5 h-3.5 text-yellow-400" />
                  <p className="text-[10px] font-black uppercase tracking-widest text-gray-500">Domain</p>
                </div>
              ...
              <div className="bg-black/60 p-4 rounded-2xl border border-[#222222]/40">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest text-gray-500">Program Type</p>
                </div>
                <p className="text-white font-bold text-sm">{cert.type}</p>
              </div>
              <div className="bg-black/60 p-4 rounded-2xl border border-[#222222]/40 col-span-2">
                <div className="flex items-center gap-2 mb-1">
            ...
            <div className="flex items-center justify-between pt-5 border-t border-[#222222]/50">
              <div className="flex items-center gap-2">
                <ShieldCheck className="w-4 h-4 text-yellow-400" />
                <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Cert ID</span>
              </div>
              <span className="bg-gradient-to-r from-yellow-300 to-yellow-400 bg-clip-text text-transparent font-black tracking-wider text-sm">{cert.id}</span>
            </div>
          </div>

          {/* Action Panel */}
          <div className="md:col-span-5 bg-[#111111]/80 backdrop-blur-md rounded-3xl border border-[#222222]/50 p-8 flex flex-col justify-center space-y-4 shadow-[0_8px_30px_rgb(0,0,0,0.3)]">
            <p className="text-[10px] font-black uppercase tracking-widest text-gray-500 text-center mb-2">Certificate Actions</p>
            <button
              onClick={handleDownloadPDF}
              disabled={downloadingPDF}
              className="w-full flex items-center justify-center gap-3 bg-gradient-to-r from-yellow-400 to-yellow-500 hover:from-yellow-300 hover:to-yellow-400 disabled:opacity-70 text-white font-bold py-4 rounded-xl transition-all shadow-lg shadow-yellow-500/25 cursor-pointer"
            >
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-3 bg-[#1A1A1A] hover:bg-[#252B48] disabled:opacity-70 text-white font-bold py-4 rounded-xl transition-all border border-[#222222] cursor-pointer"
            >
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-3 bg-[#1A1A1A] hover:bg-[#252B48] text-white font-bold py-4 rounded-xl transition-all border border-[#222222] cursor-pointer"
            >
              {copied ? <Check className="w-5 h-5 text-yellow-400" /> : <Copy className="w-5 h-5" />}
              {copied ? 'Link Copied!' : 'Copy Verification Link'}
            </button>

            <div className="pt-4 border-t border-[#222222]/50 text-center">
              <p className="text-[10px] text-gray-500 font-medium">Issued by Carrezza Global Solutions</p>
        ...
        {/* Footer branding */}
        <div className="mt-16 text-center text-xs text-gray-600 no-print">
